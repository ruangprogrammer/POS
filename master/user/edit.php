...
$queryRowUser = mysqli_query($mysqli,"SELECT * FROM users WHERE user_id = '".$id."'");
...
if (isset($_POST['ubah'])) { 
  ...
  if(($rowUser['user_level'] == 'Admin') AND ($rowUser['user_id'] =='31')){
   echo "<script> alert('Mohon Maaf data Level Administrator tidak bisa di edit'); location.href='index.php?hal=master/user/list' </script>";exit;
 }
 ...
 if (!empty($_FILES) && $_FILES['user_foto']['size'] >0 && $_FILES['user_foto']['error'] == 0 ){ 
  ...
  if($move){ 
    $queryUpdate  = mysqli_query($mysqli,"UPDATE users SET 
      user_foto = '".$foto."',
